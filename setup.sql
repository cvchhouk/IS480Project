...
--Christopher Chhouk
--IS 480 Semester Project
--chris.chhouk@gmail.com
...
create or replace package Enroll is
  ...
  --1.5/ create valid_callnum function
  Function validate_callnum(
  p_callnum schclasses.callnum%type)
  RETURN varchar2;
  --2/ Checked if the student is already enrolled
  Procedure already_enrolled(
  p_snum in students.snum%type,
  p_callnum in enrollments.callnum%type,
  p_error_text out varchar2);
  --3/ Double Enrollment: A student cannot enroll in other section of the same course unless he/she withdraw from another section
  Procedure double_enrollment(
  p_snum in students.snum%type,
  p_callnum in enrollments.callnum%type,
  p_error_text out varchar2);
  --4/ 15-hour RULE: create procedure to test total credit hours of the student do not exceed 15 credit hours
  Procedure validate_total_credit_hour(
  p_snum enrollments.snum%type,
  p_callnum schclasses.callnum%type,
  p_error_text out varchar2);
  --5/ Standing Requirement: a student's standing must be equal or higher than the standing requirement required by the courses
  Procedure standing_requirement(
  p_snum enrollments.snum%type,
  p_callnum enrollments.callnum%type,
  p_error_text out varchar2);
  --6/ Disqualified student: when a non-freshman (Standing other than 1) student's GPA is lower than 2.0, the student is now in disqualified status.
  Procedure disq_student(
  p_snum enrollments.snum%type,
  p_error_text out varchar2);
...
create or replace Package Body Enroll is
  ...
  Procedure disq_student(
    ...
    p_error_text out varchar2) as
    v_disq_gpa number(1);
    v_disq_standing number(1);
  begin
    select gpa, standing
    into v_disq_gpa, v_disq_standing
    from students
    where snum = p_snum;
    -- Check to see if GPA is lower than 2.0 and if standing is other than 1.
    if v_disq_gpa >= 2.0 or v_disq_standing = 1 then
      p_error_text := null;
    else
      p_error_text := 'Student GPA is under 2.0 and is disqualified from adding any courses.';
    end if;
  end;
  --7/ create valid_class_capacity function
  Function valid_class_capacity(
